...
################################################################################################
# NATIVE PYTORCH REPLACEMENTS FOR FASTAI HELPERS
# This ensures the backend does not crash due to FastAI versioning issues.
...
def listify(p=None):
    if p is None: return []
    elif isinstance(p, str): return [p]
    elif hasattr(p, '__iter__'): return list(p)
    else: return [p]
...
class SqueezeExcite1d(nn.Module):
    ...
    def forward(self, x):
        ...
        s = torch.sigmoid(torch.matmul(self.w2, intermed)) 
        return s * x 

def create_head1d(nf:int, nc:int, lin_ftrs=None, ps=0.5, bn_final:bool=False, bn:bool=True, act="relu", concat_pooling=True):
    "Model head that takes `nf` features, runs through `lin_ftrs`, and outputs `nc` classes"
...
################################################################################################
# BASIC CONVOLUTIONAL ARCHITECTURE ENGINE
...
################################################################################################
# FACTORY FUNCTION FOR YOUR SPECIFIC MODEL

def fcn_wang(num_classes=71, input_channels=12, lin_ftrs_head=[128], ps_head=0.5, bn_final_head=False, bn_head=True, act_head="relu", concat_pooling=True):
    # Added lin_ftrs_head=[128] as the default to match your saved weights
    return basic_conv1d(
        filters=[128, 256, 128],
        kernel_size=[8, 5, 3],
        stride=1,
        ...
        act="relu",
        bn=True,
        lin_ftrs_head=lin_ftrs_head, # This will now pass the [128] layer
        ps_head=ps_head,
